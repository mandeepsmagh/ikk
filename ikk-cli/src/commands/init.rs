...
pub enum ShellArg { Zsh, Bash, Fish, Nushell, Powershell }
...
pub async fn run(args: InitArgs, home: &IkkHome) -> Result<()> {
    ...
    let remote = if args.silent {
        args.remote.clone()
    } else {
        prompt_remote(args.remote.as_deref())?
    };
    ...
    if !args.no_shell {
        ...
        if args.dry_run {
            ...
            match install_path_integration(&shell, &home.bin_dir())? {
                true  => println!("added {} to PATH in {:?}", home.bin_dir().display(), shell.rc_file()),
